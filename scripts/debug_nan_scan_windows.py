# scripts/debug_nan_scan_windows.py
import os
import sys
...
N_CAL_WINDOWS = 4   # match calibrate_mu_from_telemetry.py's current setting

vehicle = DifferentiableMultiBodyVehicle(VP_DICT, TP_DICT)
setup = vehicle._default_setup_vec
rng = np.random.default_rng(0)   # SAME seed as calibrate_mu_from_telemetry.py
...
for f in files:
    print(f"\n=== {f.name} ===")
    df = decode_can_csv_to_dataframe(f, dbc_path=Path("TER.dbc"), dt=0.005)
    ...
    for w in idx:
        ...
        print(f"  window {w:5d}  vx0={vx0:+.2f} vy0={vy0:+.2f} wz0={wz0:+.3f}  "
              f"u[0:3]={u_all[s]}")
        ...
        try:
            for i in range(WINDOW_LEN):
                ...
                print(f"    -> OK (250 steps)")
        except Exception as ex:
            print(f"    -> CRASHED at step {i}, u={u}")
            raise